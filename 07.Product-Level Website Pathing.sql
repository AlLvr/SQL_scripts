...
WHERE created_at < '2013-04-06' -- date of request
	AND created_at > '2012-10-06' -- 3 mon before launch
    AND pageview_url = '/products';
SELECT * FROM products_pageviews;

-- Step 2: findning the next pageview_id that occurs AFTER the/product pageview
...
CREATE TEMPORARY TABLE sessions_w_next_preview_url
SELECT
...
SELECT * FROM sessions_w_next_preview_url;
...
FROM sessions_w_next_preview_url
