...
-- my version (looks like more obvious...):
SELECT
	YEAR(website_sessions.created_at) AS yr,
    MONTH(website_sessions.created_at) AS mon,
...
GROUP BY yr, mon, campaign;


/* 3. While we’re on Gsearch, could you dive into nonbrand, and pull
monthly sessions and orders split by device type? I want to flex
our analytical muscles a little and show the board we really know our traffic sources. */
SELECT
	YEAR(website_sessions.created_at) AS yr,
    MONTH(website_sessions.created_at) AS mon,
...
-- first, finding the various utm sources and referers to see the traffic we're getting (all channels)
-- all 3 are NULL = direct type in traffic / only both utm (paid parameters, not tagged by) are NULL = organic search traffic
SELECT DISTINCT
	utm_source,
    utm_campaign,
    http_referer
...
-- for this step, we'll find the first pageview_id when test started
SELECT
...
-- as the opposite of the very first step with MIN pageview_id when test started (JOIN is needed for WHERE clause)
SELECT
...
/* 7. For the landing page test you analyzed previously, it would be great
to show a full conversion funnel from each of the two pages to orders.
...
WHERE website_pageviews.created_at BETWEEN '2012-09-10' AND '2012-11-10' -- prescribed in assignment
